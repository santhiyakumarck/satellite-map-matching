...
sys.path.insert(
    0,
    "/home/sandiya/sept 3-Drone-map-test/SuperGluePretrainedNetwork"
)
...
#
# If camera is tilted significantly, this footprint
...
#
# Original satellite ROI is NEVER modified.
# Only a temporary copy is resized.
...
# ============================================================
# CREATE SUPERPOINT / SUPERGLUE MODEL
# ============================================================
...
def run_superglue(
    ...
    frame_keypoint_count = (
        len(keypoints0)
    )

    satellite_keypoint_count = (
        len(keypoints1)
    )
...
def main():
    ...
    print()
    print(
        "Loading SuperPoint + SuperGlue model..."
    )
    ...
    matching = create_matching_model(
        device
    )
    ...
    print(
        ...
        f"{model_load_time:.6f} s"
    )

    # ========================================================
    # FRAME TENSOR PREPROCESSING
    # ========================================================
    ...
    frame_preprocess_time = (
        time.perf_counter()
        - frame_preprocess_start
    )

    # ========================================================
    # SATELLITE TENSOR PREPROCESSING
    # ========================================================
    ...
    print()
    print(
        f"Frame preprocessing: "
        f"{frame_preprocess_time:.6f} s"
    )

    print(
        f"Satellite preprocessing: "
        f"{satellite_preprocess_time:.6f} s"
    )
    ...
    print()
    print(
        f"SuperPoint keypoints (frame): "
        f"{frame_keypoint_count}"
    )

    print(
        f"SuperPoint keypoints (satellite): "
        f"{satellite_keypoint_count}"
    )
    ...
    # ========================================================
    #
    # This is the important timing for your algorithm.
    #
    # It does NOT include:
    # - model loading
    # - CSV loading
    ...
    algorithm_hz = (
        1.0 / algorithm_processing_time
        if algorithm_processing_time > 0
        else 0.0
    )
    ...
    print(
        f"Satellite resize time       : "
        f"{satellite_resize_time:.6f} s"
    )

    print(
        f"Frame preprocessing time    : "
        f"{frame_preprocess_time:.6f} s"
    )

    print(
        f"Satellite preprocessing time: "
        f"{satellite_preprocess_time:.6f} s"
    )
    ...
    print()
    print(
        f"ALGORITHM PROCESSING TIME   : "
        f"{algorithm_processing_time:.6f} s"
    )

    print(
        f"ALGORITHM RATE              : "
        f"{algorithm_hz:.3f} Hz"
    )
